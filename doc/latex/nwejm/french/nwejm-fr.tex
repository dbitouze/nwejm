...
\NewDocumentCommand{\myarrow}{}{{\fontfamily{pzd}\fontencoding{U}\fontseries{m}\fontshape{n}\selectfont\char213}}
...
\newtheorem{expérience}
\newtheorem[title=contraventionnalisation]{ctrv}
\newtheorem[refname={anneau,anneaux}]{anneau}
\newtheorem[refname={idéal,idéaux}]{idéal}
%
...
\begin{dbwarning}{Compatibilité ascendante cassée avec la version~2.0}{}
  Entre les versions 1.6 et 2.0 (publiées respectivement les 8 février 2024 et
  10 février 2026) des classes \textsf{nwejm} et \nwejmauthor{}, ci-dessous
  documentées, la compatibilité ascendante a été cassée (au profit d'une
  meilleure mise en page) en termes à la fois :
  \begin{itemize}
  \item de syntaxe (cf. la clé \refKey{refname} qui remplace la clé
    \refAux{title-plural}, désormais obsolète) ;
  \item de mise en page.
  \end{itemize}
  Afin de pouvoir recompiler à l'identique d'anciens documents, il suffira de
  charger ces classes au moyen de, respectivement :
\begin{preamblecode}
...
\begin{dbwarning}{Packages nécessaires}{}
  ...
  \item du programme
    \href{https://www.ctan.org/pkg/biber}{\program{biber}}\footnote{Attention !
      ...
      avertissement~\ref{wa-affichage-auteurs} \pageref{wa-affichage-auteurs} et
      \vref{sec-bibliographie}.}.
  \end{itemize}
...
\begin{dbwarning}{Affichage des prénoms et noms du ou des
    auteurs}{affichage-auteurs}
...
\end{bodycode}
  ...
  \begin{dbremark}{Caractères accentués dans les noms d'environnements :
      acceptés}{}
    Les caractères accentués sont acceptés dans les noms d'environnements, comme
...
\end{preamblecode}
\begin{bodycode}[listing and text,listing options={deletekeywords={[2]label}}]
\begin{expérience}\label{un}
  On lance un dé à 6 faces [...]
\end{expérience}
\begin{expérience}\label{deux}
...
\end{bodycode}
  ...
  final à \meta{nom}. Si cette forme plurielle doit être construite
  différemment, il convient de le spécifier en \meta{option(s)} au moyen de la
  clé \refKey{refname} suivante.
  \begin{docKey}[][doc new=2026-02-06]{refname}{=\brackets{\meta{forme singulière},\meta{forme
          plurielle}}}{pas de valeur\\par défaut, initialement vide}
    Cette clé permet de spécifier les formes singulière et plurielle du titre
    récurrent.
  \end{docKey}
\begin{preamblecode}
\newtheorem[refname={anneau,anneaux}]{anneau}
...
\end{bodycode}
  ...
  Si la valeur du titre récurrent doit être différente de \meta{Nom}, il suffit
  de spécifier celle souhaitée au moyen de la clé \refKey{title} suivante.
...
\end{preamblecode}
\begin{bodycode}[listing and text,listing options={deletekeywords={[2]label}}]
\begin{ctrv}\label{ctrv-un}
  L'infraction constatée [...]
\end{ctrv}
\begin{ctrv}\label{ctrv-deux}
  Ici, le délit constaté [...]
\end{ctrv}
Les \vref{ctrv-un,ctrv-deux} mettent en évidence [...].

La \vref{ctrv-un} met en évidence [...].
\end{bodycode}
...
Les commandes \refCom{cref} et \refCom{vref} suivantes rendent puissante la
...
\begin{docCommand}{cref}{\brackets{\meta{label$_1$},...,\meta{label$_n$}}}
  ...
  \item en détecte les natures\objets{} et affiche avant leurs numéros les mots
    clés correspondants\footnote{Fonctionnalité fournie en sous-main par le
      \Package{zref-clever}.}, automatiquement traduit dans la langue de
...
\begin{docCommand}{vref}{\brackets{\meta{label$_1$},...,\meta{label$_n$}}}
  ...
  \item affiche\footnote{Ou pas selon le contexte.} après les numéros des objets
    référencés les (numéros de) pages où ils se situent\footnote{Fonctionnalité
      fournie en sous-main par le \Package{zref-vario} (qui s'appuie lui-même sur
      le \Package{varioref}).}.
  \end{itemize}
...
\item[\package{glossaries} :] création de glossaires et (listes d')acronymes :
  ...
  \end{description}
\item[\package{zref-clever} :] références croisées
  intelligentes\footnote{Cf. \vref{sec-references-croisees}.}.
\item[\package{zref-varioref} :] références de pages intelligentes.
\end{description}
