...
%
\DTMsavedate{release-date}{2026-02-10}
\newcommand{\releaseversion}{2.0}
...
\newtheorem{exposé}
\newtheorem[refname={criterion,criteria}]{criterion}
\newtheorem[title=random experiment]{rexp}
%
...
\begin{dbwarning}{Backward compatibility broken with version 2.0}{}
  Between versions 1.6 and 2.0 (released respectively February 8, 2024 and
  February 10, 2026) of \textsf{nwejm} et \nwejmauthor{} classes, documented
  below, backward compatibility has been broken (in favor of better layout) in
  terms of both:
  \begin{itemize}
  \item syntax (see the \refKey{refname} key, which replaces the now obsolete
    \refAux{title-plural} key);
  \item layout.
  \end{itemize}
  In order to be able to recompile old documents identically, simply load these
  classes using, respectively:
\begin{preamblecode}
\documentclass[...]{nwejm}[=2024-02-08]
\end{preamblecode}
  and
\begin{preamblecode}
\documentclass[...]{nwejmart}[=2024-02-08]
\end{preamblecode}
\end{dbwarning}
...
\begin{dbwarning}{Required packages}{}
  ...
  \item The
    \href{https://www.ctan.org/pkg/biber}{\program{biber}}\footnote{Attention!
      The bibliography must be compiled not with the usual \program{bibtex}, but
      with \program{biber}. More details in warning~\ref{wa-affichage-auteurs}
      page \pageref{wa-affichage-auteurs} and \vref{sec-bibliographie}.}
    program.
  \end{itemize}
...
\end{bodycode}
  ...
  \begin{dbremark}{Accented characters in environment names: accepted}{}
    Accented characters are accepted in environment names, as shown in the
    following example.
  \end{dbremark}
...
\end{preamblecode}
\begin{bodycode}[listing and text,listing options={deletekeywords={[2]label}}]
\begin{exposé}\label{one}
  This is about government corruption [...]
\end{exposé}
\begin{exposé}\label{two}
  This is about business corruption [...]
\end{exposé}
The \vref{one,two} highlight [...]
\end{bodycode}

  Note that, in the case of cross-references using the
  \refCom{vref}\footnote{Cf. \vref{sec-references-croisees}}(or \refCom{cref})
  command, the recurring title of the referenced \enquote{theorem(s)} is
  automatically added to the reference, possibly on its plural form. By default,
  this latter is obtained by adding a final \enquote{s} in \meta{name} or in
  \meta{recurring title} set in the title option. If this form should be
  constructed differently, this should be specified in \meta{option(s)} using
  the following \refKey{refname} key.

  \begin{docKey}[][doc new=2026-02-06]{refname}{=\brackets{\meta{singular form},\meta{plural form}}}{no default\\value, initially empty}
    This key is used to specify the singular and plural forms of the recurring title.
  \end{docKey}
\begin{preamblecode}
\newtheorem[refname={criterion,criteria}]{criterion}
\end{preamblecode}
...
\end{criterion}
\begin{criterion}\label{criterion-bis}
  Now, consider the following [...]
\end{criterion}
Considering \vref{criterion,criterion-bis}, we have [...]
\end{bodycode}
...
\end{preamblecode}
\begin{bodycode}[listing and text,listing options={deletekeywords={[2]label}}]
\begin{rexp}\label{rexp:one}
  A 6-sided dice is rolled [...]
\end{rexp}
\begin{rexp}\label{rexp:two}
  Two 6-sided dice are rolled [...]
\end{rexp}
The \vref{rexp:one,rexp:two} highlight [...]

The \vref{rexp:one} highlights [...].
\end{bodycode}
...
The following commands \refCom{cref} and \refCom{vref} provide powerful
...
\begin{docCommand}{cref}{\brackets{\meta{label$_1$},...,\meta{label$_n$}}}
  ...
  \item detects their nature\objets{} and displays the corresponding
    keywords\footnote{Feature provided behind the scenes by the
      \Package{zref-clever} package.} before their numbers, automatically
    translated into the article language\footnote{Feature provided by the
      \nwejmauthorcl.}.
  \end{itemize}
  %
\begin{bodycode}[listing and text]
...
\begin{docCommand}{vref}{\brackets{\meta{label$_1$},...,\meta{label$_n$}}}
  ...
  \item displays\footnote{Or not, depending on the context.} the (numbers of
    the) pages where they are located\footnote{Feature provided behind the
      scenes by the \Package{zref-vario} (which itself relies on the
      \Package{varioref}).} after the numbers of the referenced objects.
  \end{itemize}
...
\item[\package{glossaries}:] creation of glossaries and  (lists of) acronyms:
  ...
  \end{description}
\item[\package{zref-clever} :] smart cross-references\footnote{See
    \vref{sec-references-croisees}.}.
\item[\package{zref-varioref} :] smart page references.
\end{description}
