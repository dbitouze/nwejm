...
% \begin{docEnvironment*}{\meta{objet}}{}
%   Cet environnement permet de composer l'\meta{objet} de type théorème :
%   \begin{itemize}
%   \item où \meta{objet} est un de ceux listés à la 1\iere{} colonne du
%     \vref{env-theorems} ;
%   \item dont le titre récurrent est la traduction de \meta{objet} dans la langue
%     de l'article.
%   \end{itemize}
%   Ainsi, pour composer par exemple un théorème, on recourra à l'environnement
%   \docAuxEnvironment{theorem} dont le titre récurrent associé sera
%   \enquote{Th\'eor\`eme} en français, \enquote{Theorem} en anglais, etc.
% \end{docEnvironment*}
...
\refEnv{assertions} et \refEnv{hypotheses} qui s'emploient comme l'environnement
de listes numérotées \docAuxEnvironment{enumerate} habituel.
...
\begin{docCommand}{century(*)}{\marg{numéro}}
  Cette commande affiche l'ordinal de \meta{numéro}\footnote{Ce numéro doit être
...
Le \Package*{amsmath} fournit les environnements mathématiques :
\begin{itemize}
...
correspondants font l'objet de références croisées. Ceci oblige l'utilisateur à,
chaque fois, se poser la question de la numérotation et son choix peut s'avérer
caduc ultérieurement\footnote{Et donc nécessiter de passer d'un environnement
  étoilé à un non-étoilé et d'ajouter un label.}

Pour éviter à l'utilisateur ce genre de question et de choix, le
\Package{autonum} permet de n'utiliser que les environnements non-étoilés :
...
\begin{docCommand}{gls}{\marg{clé}}
  ...
  \item la 1\iere{} occurrence de cette commande dans le document affiche
    ...
    suivie entre parenthèses de sa \meta{forme courte} ;
...
\begin{docCommand}{blockcquote}{\marg{clé}\marg{extrait}}
  ...
  Les arguments optionnels permettent d'ajouter du texte \oarg{antérieur} et/ou
  \oarg{postérieur} à la citation.
\begin{bodycode}[listing and text]
...
attaché au fichier \file{PDF} de la présente documentation\footnote{Il peut
  ...
  permetttent de sélectionner et de copier du texte.}.
%
\embedfile[
  filespec=template.tex,
  desc={Template for articles to be published in NWEJM},
...
\tcbinputlisting{%
  ...
  listing only,
  listing file=../../../doc/latex/nwejm/examples/template.tex,
